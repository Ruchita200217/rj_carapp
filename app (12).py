...
features = pd.DataFrame(features, columns=['buying', 'maint', 'doors', 'persons', 'lug_boot', 'safety'])
# Apply mapping to features using a dictionary comprehension
features = features.apply(lambda col: col.map({val: i for i, val in enumerate(df[col.name].unique())}))
